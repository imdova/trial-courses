...
const EmailVerification = ({ onComplete }: EmailVerificationProps) => {
  const { email, setAuthOtp, incrementAuthAttempts } = useVerification();
  ...
  const verifyOtp = async () => {
    ...
    try {
      ...
      if (isValid) {
        setAuthOtp(otpString); // Set the OTP in the context
        setIsVerified(true);
        setTimeout(() => {
          onComplete();
        }, 1000);
  ...
  return (
    <div className="max-w-md mx-auto">
      ...
      <p className="text-gray-600 mb-8">
        ...
        <span className="font-medium text-gray-800">{email}</span>. The code
