...
const BlockFormContent: React.FC = () => {
  const {
    getActiveBlock,
    updateBlock,
    forms,
  ...
  const fieldChange = (field: FormFieldConfig) => {
    updateBlock({
      ...block,
      fields: block?.fields?.map((f) => (f.id === field.id ? field : f)),
    });
  };
  const formDataChange = (formData: Partial<Block["formData"]>) => {
    updateBlock({
      ...block,
      formData: {
        ...block?.formData,
  ...
  const handleDuplicate = (fieldId: string) => {
    ...
    updateBlock({
      ...
      fields: [
        ...
        {
          ...field,
          name: uniqueName(block?.fields?.map((f) => f.name) || [], field.name),
          id,
        },
      ],
    });
  ...
  return (
    <div>
      ...
      <div className="space-y-2">
        ...
        <TextareaAutosize
          name="Description"
          value={block?.formData?.description}
      ...
      <h4 className="text-muted-foreground mt-10 text-center text-xs">
        Fields
      </h4>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <input
            name="apiEndpoint"
            type="text"
            value={block?.formData?.apiEndpoint}
        ...
        <div className="flex items-center justify-between">
          ...
          >
            <span
              className={`line-clamp-1 w-full text-left text-xs ${block?.formData?.method ? "text-main" : "text-muted-foreground"}`}
            >
              {block?.formData?.method || "POST"}
            </span>
          </DropDown>
        </div>
        ...
        <Selector
          label="After Submit"
          value={block?.formData?.afterSubmit}
          placeholder="Select After Submit"
          onChange={(value) =>
            formDataChange({
              afterSubmit: value as
                | "close"
                | "open popup"
                | "redirect"
                | "none",
        ...
        {block?.formData?.afterSubmit === "open popup" && (
          <>
            <Selector
              label="Form Id"
              value={block?.formId}
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <input
            ...
            type="text"
            value={block?.formData?.onSuccessRedirect}
            onChange={(e) =>
        ...
        <div className="flex items-center justify-between">
          ...
          <input
            ...
            type="text"
            value={block?.formData?.onSuccessMessage}
            onChange={(e) =>
        ...
        <div className="flex items-center justify-between">
          ...
          <input
            ...
            type="text"
            value={block?.formData?.onErrorRedirect}
            onChange={(e) =>
        ...
        <TextareaAutosize
          name="onErrorMessage"
          value={block?.formData?.onErrorMessage}
          minRows={2}
...
const FieldForm = ({
  ...
  drag(dragRef);
  preview(previewRef);
  // TODO : make sure name is not duplicate while adding it
...
export default BlockFormContent;
