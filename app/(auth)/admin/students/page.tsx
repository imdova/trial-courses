...
import OverviewStudentsPage from "./panels/OverviewStudents";
import StudentList from "./panels/StudentsList";
...
export default function StudentsPage() {
  ...
  const [studentSettings, setStudentSettings] = useState<InstructorSetting[]>([
    // Degrees
  ...
  const categories = studentSettings
  ...
  const handleSettingSubmit = (data: {
    ...
    setStudentSettings([...studentSettings, newSetting]);
  };
  ...
  const handleConfirmDelete = () => {
    ...
    setStudentSettings(studentSettings.filter((s) => s.id !== settingToDelete.id));
  ...
  return (
    <Tabs defaultValue="Students List" className="px-3">
      <Card className="flex w-full flex-col items-center justify-between gap-3 space-y-0 p-2 md:flex-row">
        ...
        <TabsTrigger value="Students List">Students List</TabsTrigger>
          <TabsTrigger value="Students Overview">Students Overview</TabsTrigger>
          <TabsTrigger value="Students Settings">Students Settings</TabsTrigger>
        </TabsList>
      ...
      <TabsContent value="Students Overview">
        <OverviewStudentsPage />
      </TabsContent>
      <TabsContent value="Students List">
        <StudentList />
      </TabsContent>
      
      <TabsContent value="Students Settings" className="space-y-6">
        ...
        <AlertDialog open={settingDeleteDialogOpen} onOpenChange={setSettingDeleteDialogOpen}>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Delete Student Setting</AlertDialogTitle>
              <AlertDialogDescription>
                ...
                This action cannot be undone and may affect existing student profiles.
              </AlertDialogDescription>
            </AlertDialogHeader>
